...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    @When("a Get call is made to get all  the employee job titles")
    public void a_get_call_is_made_to_get_all_the_employee_job_titles() {
    ...
    @Then("the status code for getting the employees job titles is {int}")
    public void the_status_code_for_getting_the_employees_job_titles_is(Integer statusCode) {
        response.then().assertThat().statusCode(statusCode);
    ...
    public void a_patch_call_is_made_to_partially_update_the_employee() {
        ...
        response.then().assertThat().
                body("employee", equalTo("emp_firstname"));
        response.then().assertThat().
                body("Employee.emp_firstname",equalTo("sabawoon"));
        response.then().assertThat().header("Connection",equalTo("Keep-Alive"));


    }
    @Then("the status code for this partial updating  is {int}")
    public void the_status_code_for_this_partial_updating_is(Integer statusCode) {
        response.then().assertThat().statusCode(statusCode);
        response.prettyPrint();
    }
